...
$('#varsImageReviewButton').on('click', () => {
    ...
    window.location.href = `common/vars?sequence=${sequences.join('&sequence=')}`;
});
...
$('#varsQaqcButton').on('click', () => {
    ...
    $('#load-overlay').addClass('loader-bg');
    const sequences = new FormData($('#varsIndexForm')[0]).getAll('sequence');
    window.location.href = `../../qaqc/static/vars/checklist?sequence=${sequences.join('&sequence=')}`;
});
...
$('#tatorImageReviewButton').on('click', () => {
    ...
    window.location.href = `common/tator?project=${formData.get('project')}&section=${formData.get('section')}&deployment=${formData.getAll('deployment').join('&deployment=')}`;
});
...
$('#tatorQaqcButton').on('click', () => {
    ...
    $('#load-overlay').addClass('loader-bg');
    const formData = new FormData($('#tatorIndexForm')[0]);
    window.location.href = `../../qaqc/static/tator/checklist?project=${formData.get('project')}&section=${formData.get('section')}&deployment=${formData.getAll('deployment').join('&deployment=')}`;
});
