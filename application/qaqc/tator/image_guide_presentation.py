...
class ImageGuidePresentation:
    ...
    def build(self, records: list[dict]):
        ...
        while i < len(records):
            ...
            for j in range(6):
                ...
                if localization.get('attracted') is None or localization['attracted'] == 'Not Attracted':
    ...
    def _fetch_normalized_image(self, localization: dict) -> BytesIO:
        ...
        frame_bytes = self._get_frame(localization['media_id'], frame=localization['frame'])
    ...
    def _get_frame(self, media_id: int, frame: int = None) -> bytes:
        url = f'{self.tator_url}/rest/GetFrame/{media_id}'
        params = {}
