        ...
        for localization in localizations:
            ...
            if scientific_name not in phylogeny.keys():
                ...
                if worms_id_res.status_code == 200 and worms_id_res.json() != -999:  # -999 means more than one matching record
                    ...
                    worms_id_res = requests.get(url=f'https://www.marinespecies.org/rest/AphiaClassificationByAphiaID/{aphia_id}')
                    if worms_id_res.status_code == 200:
                        phylogeny[scientific_name] = flatten_taxa_tree(worms_id_res.json(), {})
                    ...
                    if worms_name_res.status_code == 200 and len(worms_name_res.json()) > 0:
                        ...
                        for record in worms_name_res.json():
                            if record['status'] == 'accepted':
                                worms_id_res_2 = requests.get(url=f'https://www.marinespecies.org/rest/AphiaClassificationByAphiaID/{record["AphiaID"]}')
                                if worms_id_res_2.status_code == 200:
                                    phylogeny[scientific_name] = flatten_taxa_tree(worms_id_res_2.json(), {})
